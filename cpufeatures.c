...
printf("cpufeatures.c: host unix\n");
#if HAVE_AUXV_HWCAP_H

printf("cpufeatures.c: HAVE_AUXV_HWCAP_H  support\n");
...
printf("cpufeatures.c: no HWCAP support\n");
#if HAVE_SYSCTLBYNAME
    int64_t valueFromSysctl = 0;
    size_t sz = sizeof(valueFromSysctl);

    printf("Detecting CPU features using sysctlbyname\n");
...
int main(void) {
  if (minipal_getcpufeatures() & IntrinsicConstants_Invalid) printf("invalid\n");
printf("minipal_getcpufeatures() -> %d\n", minipal_getcpufeatures());
}
